...
declare
  ...
  -- 3. Everything hanging off a game, keyed by `game_id`.
  children constant text[] := array[
    'game_players', 'game_turns', 'game_stages', 'game_stage_scores',
    'game_stage_passes', 'game_milestones', 'game_extensions', 'score_events',
    'dice_rolls'
  ];
...
begin
  ...
  for pol in
    ...
    where schemaname = 'public'
      and tablename = any (
        array(select unnest(plain[:][1:1])) || scoring || children || array['games']
      )
  loop
...
begin
  ...
  if before_row - identity is distinct from after_row - identity
